      ...
      <Card className="space-y-3">
        ...
        {arView ? (
          <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-black/40">
            <iframe
              src={`/ar.html?points=${selectedVariant?.coords
                .map((c) => `${c[1]},${c[0]}`) // Convert [lon, lat] to lat,lng
                .join(';')}`}
            ...
            <div className="absolute inset-x-0 bottom-0 p-3">
              <div className="rounded-2xl border border-white/10 bg-black/70 px-3 py-2 backdrop-blur">
                <div className="text-xs text-zinc-400">AR Guidance Active</div>
                <div className="text-sm font-semibold">Follow the pointers on your screen</div>
              </div>
            </div>
          </div>
        ) : (
