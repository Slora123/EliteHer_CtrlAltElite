import { n as WebPlugin } from "./dist-BSPKxkRt.js";
//#region node_modules/@capacitor/geolocation/dist/esm/web.js
var GeolocationWeb = class extends WebPlugin {
	constructor() {
		...
		if (typeof window !== "undefined") {
			const win = window;
			if ("ondeviceorientationabsolute" in win) win.addEventListener("deviceorientationabsolute", (event) => this.updateOrientation(event, true), true);
			else if ("ondeviceorientation" in win) win.addEventListener("deviceorientation", (event) => this.updateOrientation(event, false), true);
		}
	}
	updateOrientation(event, isAbsolute) {
		...
		if (isAbsolute && event.alpha !== null) trueHeading = (360 - event.alpha) % 360;
		...
		} else if (event.alpha !== null && event.absolute === true) trueHeading = (360 - event.alpha) % 360;
		else if (event.alpha !== null) magneticHeading = (360 - event.alpha) % 360;
		if (trueHeading !== null || magneticHeading !== null) this.latestOrientation = {
			trueHeading,
			magneticHeading,
			headingAccuracy
		};
...
new GeolocationWeb();
//#endregion
export { GeolocationWeb };

//# sourceMappingURL=web-DNsngMrw.js.map